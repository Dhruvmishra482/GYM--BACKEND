...
exports.addMember = async (req, res, next) => {
  try {
    ...
    if (owner.usageStats.membersCount !== actualMemberCount) {
      console.log(`Syncing member count before addition: ${owner.usageStats.membersCount} → ${actualMemberCount}`);
    ...
    if (planLimits.members !== -1 && owner.usageStats.membersCount >= planLimits.members) {
      return res.status(403).json({
        success: false,
    ...
    console.log('Full member object:', JSON.stringify(member.toObject(), null, 2));
    console.log('===========================');

    // CRITICAL: Increment member count immediately after successful creation
    await owner.incrementMemberCount();
    console.log(`Member added successfully. Updated count: ${owner.usageStats.membersCount + 1}`);
...
exports.deleteMember = async (req, res) => {
  try {
    ...
    if (owner) {
      ...
      console.log(`Member deleted. Updated count: ${owner.usageStats.membersCount - 1}`);
    }
...
exports.deleteMemberById = async (req, res) => {
  try {
    ...
    if (owner) {
      await owner.decrementMemberCount();
      console.log(`Member deleted by ID. Updated count: ${owner.usageStats.membersCount - 1}`);
    }
